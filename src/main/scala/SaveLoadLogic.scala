...
object SaveLoadLogic {
    ...
    // GRAVAR – agora com selected opcional (padrão None para compatibilidade com TUI)
    def saveGameState(board: Board, size: Int, currentPlayer: Stone, timeLimit: Long,
                      ...
                      selected: Option[Coord2D] = None): String = {
    ...
    // CARREGAR – suporta formato antigo (8 linhas) e novo (9 linhas)
    def loadGameState(data: String): Option[(Board, Int, Stone, Long, List[Coord2D], MyRandom, Option[Stone], Option[Coord2D])] = {
        val lines = data.split("\n")

        lines.toList match {
            case sizeStr :: timeLimitStr :: playerStr :: seedStr :: openCoordsStr :: boardStr :: humanStr :: selectedStr :: Nil =>
                for {
                ...
                } yield (board, size, player, timeLimit, openCoords, MyRandom(seed), humanColor, selected)

            case sizeStr :: timeLimitStr :: playerStr :: seedStr :: openCoordsStr :: boardStr :: humanStr :: Nil =>
                // Formato antigo (8 linhas) – selected = None
                for {
                    size <- sizeStr.toIntOption
                    timeLimit <- timeLimitStr.toLongOption
                    player <- if (playerStr == "B") Some(Stone.Black) else if (playerStr == "W") Some(Stone.White) else None
                    ...
                    board = boardList.foldLeft(ParMap[Coord2D, Stone]())((acc, elem) => acc + elem)
                } yield (board, size, player, timeLimit, openCoords, MyRandom(seed), humanColor, None)
